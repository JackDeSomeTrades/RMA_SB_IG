...
class ForwardTask(EnvScene, BaseTask):
    ...
    def _setup_task_tensors(self):
        ...
        self.max_episode_length = np.ceil(self.max_episode_length_s / self.dt)  # in terms of timsesteps.
        self._elapsed_steps = None  # to count the number of times the step function is called.
        ...
        self.base_rpy = get_euler_xyz(
            self.base_quat)  # provides (r, p, y) tuple of the base torso with each r,p,y of size num_envs
...
class VarForwardTask(VariantEnvScene, BaseTask):
    def __init__(self, *args):
        config = args[0][0]
        sim_params = args[0][1]
        args = args[0][2]
        ...
        self.init_done = False
        BaseTask.__init__(self, cfg=config, sim_params=sim_params, sim_device=args.sim_device)
        VariantEnvScene.__init__(self, cfg=config, physics_engine=args.physics_engine, sim_device=args.sim_device,
                                 headless=args.headless, sim_params=sim_params)
        set_seed(config.seed)
