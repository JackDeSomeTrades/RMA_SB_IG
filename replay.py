...
import argparse
from utils.helpers import get_config, get_project_root, parse_config
from box import Box
...
def get_updated_params_for_replay(cfg):
    env_cfg, sim_params, env_args = parse_config(cfg)
...
def play():
    ...
    parser.add_argument('--load_save', '-l', type=str)

    args = parser.parse_args()

    cfg = get_config(f'{args.cfg}_conf.yaml')
    policy_save_fpath = os.path.join(os.getcwd(), args.load_save)

    # prepare environment
    replay_env = RMAA1TaskVecEnvStableBaselineGym(get_updated_params_for_replay(cfg))
